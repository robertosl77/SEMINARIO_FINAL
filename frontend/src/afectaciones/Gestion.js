...
function Gestion({ cuenta, idafectacion, telefonos = [], solucion_provisoria = [], onGestionChange, onClose }) {
  ...
  const handleContactoSubmit = () => {
    ...
    fetch(`https://seminario-final.onrender.com/API/GE/AgregaContacto`, {
      method: 'POST',
      headers: {
  ...
  const handleSolucionChange = (event) => {
    ...
    fetch(`https://seminario-final.onrender.com/API/GE/CambiaGestion/${cuenta}/${idafectacion}/${nuevaSolucion}`, {
      method: 'POST',
    })
      .then(response => response.json())
      .then(data => {
